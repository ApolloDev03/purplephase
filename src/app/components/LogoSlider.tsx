                ...
                >
                    {marqueeItems.map((logo, index) => (
                        <motion.div
                            ...
                            className="relative z-0 flex h-25 w-42.5 shrink-0 cursor-pointer items-center justify-center rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-shadow duration-300 hover:z-10 hover:shadow-lg md:h-[150px] md:w-[250px]"
                        >
                            <img
                                src={logo.client_logo}
                                alt={logo.altTag || logo.client_name}
                                className="max-h-25 max-w-full object-contain transition duration-300 md:max-h-[100px]"
                            />
                        </motion.div>
                    ))}
                </motion.div>
...
export function LogoSlider() {
    ...
    return (
        <section
            ref={sectionRef}
           className="w-full overflow-x-hidden bg-[#f6f6f6] py-8 md:py-10 lg:py-[85px]"
        >
            {loading ? (
                <div className="flex min-h-55 items-center justify-center">
                    <div className="h-12 w-12 animate-spin rounded-full border-4 border-white border-t-[#A62666]"></div>
                </div>
